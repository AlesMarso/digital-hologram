...
const uint MAX_POINTS_COUNT = 4096;
...
void program1()
{
	...
	float val = 0;
	vec4 pixelVal;
	float max_ampl = -100000000;
	float max_phase = -100000000;
...
shared vec2 arr[4096];

void fft_sort_4096_points(uint id_points_array)
{
	uint texPosX = gl_LocalInvocationID.x;
	uint pixelsCount =  MAX_POINTS_COUNT_PER_THREAD;
	uint groupId = texPosX * MAX_POINTS_COUNT_PER_THREAD;

	for(uint i = 0; i < pixelsCount; i++)
	{
		uint index = i + groupId;
		uint reverse_i = BitReverse(index, Log2Levels);
		arr[index] = imageLoad(out_tex, ivec2(reverse_i, id_points_array)).rg;
	}
	...
	{
		uint index = i + groupId;
		float pixelVal = arr[index].r;
		imageStore(out_tex, ivec2(index, id_points_array), vec4(pixelVal, pixelVal, pixelVal, pixelVal));
	}
}
